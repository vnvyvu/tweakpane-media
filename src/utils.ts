export function mergeElement<T extends HTMLElement>(
	...
	if (elementProps) {
		...
		for (let i = 0; i < keys.length; i++) {
			...
			const v = elementProps[k];
			const defaultV = defaultProps?.[k];
			...
			switch (k) {
				...
				case 'children':
					...
					else if (defaultV instanceof Node)
						resultElement.appendChild(defaultV);
					else if (defaultV) resultElement.innerHTML = defaultV.toString();
					break;
				case 'classList':
					break;
				case 'clientHeight':
				...
				case 'style': {
					...
					for (let j = 0; j < styleKeys.length; j++) {
						...
						const styleV = elementProps.style?.[styleK];
						const defaultStyleV = defaultProps?.style?.[styleK];
						...
						if (typeof styleV !== 'function') {
							switch (styleK) {
								...
								default:
									...
									else if (defaultStyleV)
										(resultElement.style as any)[styleK] = defaultStyleV;
									break;
							}
						}
					}
